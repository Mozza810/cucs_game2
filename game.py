...
def print_menu(exits, room_items, inv_items):
    ...
    # Loop available items
    for item in room_items:
        # Print the item and instruction
        print("TAKE", item["id"].upper(), "to take", item["name"])

    for item in inv_items:
        # ID and drop instructions
...
def execute_go(direction):
    ...
    # Check if the current room exits work with the user chosen direction
...
def execute_take(item_id):
    ...
    for item in current_room["items"]:
        if(item["id"] == item_id):
            room_item = item
    if room_item:
        # If it is room item, remove from room, add to inv.
